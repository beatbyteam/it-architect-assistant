...
P1 вводит минимальные guardrails для модулей, которые уже однажды разрослись до уровня, опасного для сопровождения.
...
- orchestration и pure transformation логика должны жить в разных модулях;
- сериализация read-model и snapshot-summary не должна оставаться внутри orchestration-сервиса;
- крупные normalization-процедуры выносятся в отдельные helper-модули;
- rule executors не должны жить в одном файле с orchestration движком.
